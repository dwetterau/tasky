...
export function homepageOidc(options: { origin: string; clientId: string; clientSecret: string; loginPage: string; consentPage: string }) {
  const provider = oidcProvider({
    loginPage: options.loginPage, consentPage: options.consentPage,
    trustedClients: [{ clientId: options.clientId, clientSecret: options.clientSecret, name: "Tasky Homepage", type: "web", disabled: false, redirectUrls: [`${options.origin}/auth/callback`], metadata: null }],
    scopes: ["openid", "profile", "email", "offline_access"],
    defaultScope: "openid profile email offline_access",
    useJWTPlugin: true, requirePKCE: true, allowPlainCodeChallengeMethod: false,
  ...
  return {
    id: "homepage-oidc",
    endpoints: {
      homepageAuthorize: createAuthEndpoint("/oauth2/authorize", { method: "GET", query: z.record(z.string(), z.string()) }, async ctx => {
        if (ctx.query.client_id !== options.clientId) throw new APIError("BAD_REQUEST", { error: "invalid_client" });
        ...
        return provider.endpoints.oAuth2authorize({ ...ctx, asResponse: true });
      }),
      homepageToken: createAuthEndpoint("/oauth2/token", provider.endpoints.oAuth2token.options, async ctx => {
        ...
        if (ctx.body.client_id !== options.clientId || typeof secret !== "string" || secret.length > 1024) throw new APIError("UNAUTHORIZED", { error: "invalid_client" });
        const digest = async (value: string) => new Uint8Array(await crypto.subtle.digest("SHA-256", new TextEncoder().encode(value)));
        const [actual, expected] = await Promise.all([digest(secret), digest(options.clientSecret)]);
        let difference = 0;
        for (let index = 0; index < actual.length; index++) difference |= actual[index] ^ expected[index];
        if (difference) throw new APIError("UNAUTHORIZED", { error: "invalid_client" });
