...
export function isolateHomepageFromMcp(provider: ReturnType<typeof mcp>, homepageClientId: string) {
  return {
    ...provider,
    endpoints: {
      ...provider.endpoints,
      mcpOAuthToken: createAuthEndpoint("/mcp/token", provider.endpoints.mcpOAuthToken.options, async ctx => {
        ...
        const field = (name: string): unknown => ctx.body instanceof FormData
          ? ctx.body.getAll(name).at(-1) : ctx.body[name];
        ...
        if (credential != null) {
          ...
          if (typeof credential !== "string") throw new APIError("BAD_REQUEST", { error: "invalid_request" });
          let clientId: unknown;
          if (isRefresh) {
            const grant = await ctx.context.adapter.findOne<{ clientId: string }>({
              model: "oauthAccessToken",
              where: [{ field: "refreshToken", value: credential }],
            });
            clientId = grant?.clientId;
          } else {
            const grant = await ctx.context.internalAdapter.findVerificationValue(credential);
            if (grant) clientId = (JSON.parse(grant.value) as { clientId?: unknown }).clientId;
          }
          ...
          if (clientId === homepageClientId) throw new APIError("UNAUTHORIZED", { error: "invalid_grant" });
        }
        return provider.endpoints.mcpOAuthToken({ ...ctx, asResponse: true });
      }),
      getMcpSession: createAuthEndpoint("/mcp/get-session", provider.endpoints.getMcpSession.options, async ctx => {
        const session = await provider.endpoints.getMcpSession({
          ...ctx, asResponse: false, returnHeaders: false, returnStatus: false,
        });
        ...
        return ctx.json(session?.clientId === homepageClientId ? null : session);
      }),
    },
  };
}
