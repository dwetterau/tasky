...
export class Coordinator extends DurableObject<Env> {
    ...
    return this.queue.run(async () => {
      ...
      if (path === "/enroll") {
        ...
        if (!await this.ctx.storage.get(`user:${userId}`)) {
          const modules = Object.fromEntries(backgroundModules.map(module => [module.id, module.initialConfig(this.env)]));
          await this.ctx.storage.put(`user:${userId}`, { userId, modules });
        }
        await this.ctx.storage.setAlarm(Date.now() + 1000);
        return Response.json({ ok: true });
      }
      if (path === "/tick") { await this.tick(); return Response.json({ ok: true }); }
  ...
  private async tick() {
    ...
    const users = await this.ctx.storage.list<EnabledUser>({ prefix: "user:", limit: 25, ...(cursor ? { startAfter: cursor } : {}) });
    ...
    for (const [key, user] of users) {
      ...
      } catch { console.warn(JSON.stringify({ event: "homepage_publisher_retry" })); }
      for (const module of backgroundModules) {
        ...
        try {
          ...
          const snapshot = await module.collect({ env: this.env, userId: user.userId, config: user.modules[module.id], load: <T>() => this.ctx.storage.get<T>(stateKey), save: value => this.ctx.storage.put(stateKey, value) });
          if (snapshot) await objectCall(this.env.PUBLISHERS, user.userId, "/accept-module", { userId: user.userId, snapshot });
        } catch { console.warn(JSON.stringify({ event: "homepage_collector_retry", module: module.id })); }
      }
      await this.ctx.storage.put("cursor", key);
    }
    if (users.size < 25) await this.ctx.storage.delete("cursor");
  }
  async alarm() { await this.queue.run(() => this.tick()); }
}
