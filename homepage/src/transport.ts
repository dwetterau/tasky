...
async function hmacKey(secret: string) {
  if (!secret || secret.length < 32)
    throw new Error("Service secret is not configured");
...
export async function signedHeaders(
  ...
  return {
    ...
    "x-home-signature": bytesToBase64(new Uint8Array(signature)),
  };
}
export async function verifyRequest(
  ...
  if (!/^\d{13}$/.test(time) || Math.abs(now - Number(time)) > 300_000)
    return false;
