  ...
  async fetch(request: Request) {
    return this.queue.run(async () => {
      try {
        ...
        if (path === "/begin") {
          if (await this.ctx.storage.get("attempt"))
            throw new HttpError(409, "Login already started");
        ...
        if (path === "/remember") {
          ...
          if (
            ...
            throw new HttpError(403, "Account not enrolled");
          if (
            (await this.ctx.storage.get("grant")) ||
            (await this.ctx.storage.get("revoked"))
          )
            throw new HttpError(409, "Session already used");
          const grant: RememberedGrant = {
            sid,
            identity,
            encryptedRefresh: await encryptGrant(this.env, refreshToken, sid),
          ...
          return Response.json({
            token: await issueSession(this.env, identity.userId, sid),
          });
        }
        if (path === "/renew") {
          ...
          try {
            const next = await refreshGrant(
              this.env,
              await decryptGrant(this.env, grant.encryptedRefresh, grant.sid),
              grant.identity.userId,
            );
            grant.encryptedRefresh = await encryptGrant(
              this.env,
              next,
              grant.sid,
            );
